...
BETA0 = 1;
BETA1 = 1  ## default prior distribution
M = 30;
ONE = (1 << M);
HALF = (1 << (M - 1))
QUARTER = (1 << (M - 2));
...
def encode(string, c0=BETA0, c1=BETA1, adaptive=1, verbose=0):
    ...
    for c in string:
        ...
        if adaptive:
            ...
            pass
        boundary = a + int(p0 * w)
        if (boundary == a): boundary += 1; print("warningA");
        pass  # these warnings mean that some of the probabilities
        if (boundary == b): boundary -= 1; print("warningB");
        pass  # requested by the probabilistic model
        ...
        while (1):  ## condition at end
            ...
            if (model_needs_updating):
                ...
                if adaptive:
                    ...
                    pass
                boundary = a + int(p0 * w)
                if (boundary == a): boundary += 1; print
                "warningA";
                pass
                if (boundary == b): boundary -= 1; print
                "warningB";
                pass
